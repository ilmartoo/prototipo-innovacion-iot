...
import { getUserById, secondsToTimeString, toFixed2, toPercentageFixed2 } from "@/data/app-data";
...
export default function ActivitySummaryTab(props: ActivitySummaryDataProps) {
  ...
  const [isPositionCollapsibleOpen, setIsPositionCollapsibleOpen] = useState(
    props.report.conditions.gamePositions.reduce(
      (map, p) => ({ ...map, [p]: false }),
      {} as Record<string, boolean>
    )
  );

  const activityUsers = props.report.conditions.participants.reduce(
    (map, userId) => ({ ...map, [userId]: getUserById(userId) }),
    {} as Record<string, User>
  );

  const winnerUser = props.report.winning.player && activityUsers[props.report.winning.player];

  // Tiros de jugadores
  const playerShots = Object.keys(props.report.data.players).map((player) => {
    const shots = props.report.data.players[player].shots;
    return {
      player,
  ...
  // Tiempos de turno
  const playerTurnTimes = Object.keys(props.report.data.players).map((player) => {
    const times = Object.values(props.report.data.players[player].time.gamePosition).flat();
      ...
      <>
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard
            ...
            progress={toPercentageFixed2(playerData.time.total / props.report.data.elapsedTime)}
            progressColor="blue"
          />
          <StatCard
                      ...
                      <CardContent className="px-4">
                        <CollapsibleTrigger className="flex flex-row justify-between items-center w-full">
                          <div className="text-left">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {playerData.turns.gamePosition[position]} turno(s)
                            </p>
                          </div>{" "}
                          <Button variant="ghost" size="icon-sm" asChild>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              ...
                              <TableItem
                                ...
                                value={`${playerData.shots.gamePosition[position].in} ~ ${playerData.shots.gamePosition[position].total ? toPercentageFixed2(playerData.shots.gamePosition[position].in / playerData.shots.gamePosition[position].total) : 0}%`}
                              />
                              <TableItem
                                title="Lanzamientos al portero"
                                value={`${playerData.shots.gamePosition[position].goalkeeper} ~ ${playerData.shots.gamePosition[position].total ? toPercentageFixed2(playerData.shots.gamePosition[position].goalkeeper / playerData.shots.gamePosition[position].total) : 0}%`}
                              />
                              <TableItem
                                title="Lanzamientos por fuera"
                                value={`${playerData.shots.gamePosition[position].out} ~ ${playerData.shots.gamePosition[position].total ? toPercentageFixed2(playerData.shots.gamePosition[position].out / playerData.shots.gamePosition[position].total) : 0}%`}
                              />
                              {/* Tiempos */}
                              <TableItem
                                title="Tiempo en la posición"
                                value={secondsToTimeString(playerData.time.gamePosition[position])}
                              />
                              <TableItem
                                title="Tiempo medio por lanzamiento"
                                value={
                                  playerData.shots.gamePosition[position].total
                                    ? secondsToTimeString(
                                        playerData.time.gamePosition[position] /
                                          playerData.shots.gamePosition[position].total
                                      )
                                    : "0s"
                                }
  ...
  function renderGlobalView() {
    const playerAgg = aggregatePayerData(props.report);
    ...
    return (
      <>
        {winnerUser && (
          <>
            <SectionTitle
              ...
              subtitle={`Ha anotado un total de ${props.report.data.players[winnerUser.id].shots.in} goles.`}
            />
          </>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard value={secondsToTimeString(playerAgg.time.total)} label="TIEMPO DE ACTIVIDAD" />
          <StatCard
